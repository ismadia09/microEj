...
public class AnimationSampleWithFullRepaint extends Displayable {
	...
	public AnimationSampleWithFullRepaint(Display display) {
		...
		try {
			this.microejImage = Image.createImage("/images/microej.png");
		} catch (IOException e) {
	...
	private class HorizontalAnimatorTask extends TimerTask {
		...
		private int horizontalIncrement = this.ABSOLUTE_INCREMENT;
		...
		public HorizontalAnimatorTask(AnimationSampleWithFullRepaint animated) {
			...
			this.leftLimit = animatedImageHalfWidth;
			this.rightLimit = animated.getDisplay().getWidth() - animatedImageHalfWidth;
		}

		@Override
		public void run() {
			this.animated.imageX += this.horizontalIncrement;
			if (this.animated.imageX > this.rightLimit) {
				this.animated.imageX = this.rightLimit;
				this.horizontalIncrement = -this.ABSOLUTE_INCREMENT;
			} else {
				if (this.animated.imageX < this.leftLimit) {
					this.animated.imageX = this.leftLimit;
					this.horizontalIncrement = this.ABSOLUTE_INCREMENT;
				}
			}
	...
	public void paint(GraphicsContext g) {
		...
		// draw the image
		g.drawImage(this.microejImage, this.imageX, this.imageY, GraphicsContext.HCENTER | GraphicsContext.VCENTER);
